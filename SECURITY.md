...
| **Git sync** (`src/git-sync.js`, `src/sync.js`) | **both** `STICKIES_AUTO_SYNC` truthy **and** `STICKIES_SYNC_REPO` set | A plaintext JSON document of **all** your notes, committed to that repo — and pushed to whatever remote it has **only if you also set `STICKIES_SYNC_PUSH=1`**, which is a separate opt-in. Without it nothing is sent over the network: SessionStart pulls, and the Stop hook commits on turns that captured something. Use a **private** repo. |
...
| **Secret capture at rest** | Content **and tags** are scrubbed on write (`src/redact.js`): PEM private key blocks, including PGP (`…PRIVATE KEY BLOCK-----`) and a header that lost its footer; AWS/GitHub/Slack/Google/OpenAI/Anthropic/Stripe/npm/SendGrid/DigitalOcean token shapes and JWTs; Slack **and Discord** incoming-webhook URLs — the latter matters because Stickies itself asks you for one; `scheme://user:pass@host` connection strings; and labelled `KEY=<value>` / `"key":"<value>"` assignments (including `SCREAMING_SNAKE` env-var names) are replaced with `[REDACTED]`; the write tool/CLI warn when this happens. Also `hf_`, `glpat-`, `ASIA`, `xapp-`, `xoxe`, `GOCSPX-`, Fly `fm2_`, Azure `AccountKey=`, and opaque `Bearer` values. Redaction is a fixed point: running it over already-redacted text no longer rewrites what it previously marked, and its lookbehind is bounded, because an unbounded one took 4.7s over 100KB in a function every write path calls. `src/repo-mode/engine.mjs` needs its own self-contained copy, and that copy is now **generated** from these sources by `scripts/build-engine.mjs` and stamped with a content hash — it is no longer hand-maintained. It was hand-maintained, and it drifted nine credential patterns behind, so a repo-mode user got an older redactor than everyone else. `npm run gate` recomputes the stamp from file content (never from the literal the file declares about itself, which survives editing) and fails if the inlined copy does not match its sources. Best-effort, not a vault — an unlabelled high-entropy token with no known shape can still pass. |
...
| Surface | Notes |
|---------|-------|
| **Post-turn Stop hook** (`src/auto-capture.js`) | Parses `!!sticky …` directives from the just-finished assistant turn and persists them. Directive content flows through the same `createSticky` path → secret redaction + size/tag bounds apply; control characters are stripped first (a NUL used to truncate the note at rest and defeat dedup), tags are trimmed rather than costing the whole note, and writes are deduped on scope + content + category + importance — so an escalation (`todo P3` then `blocker P1` on the same text) is kept, and a note you deliberately dismissed is not resurrected the next time the model restates it. **Scope: project-scoped to `cwd`, with one deliberate exception — a directive carrying the `global` modifier is stored unscoped and reaches every project.** Because the parser's input is model output, and model output is shaped by whatever untrusted files, pages and tool results the model read, a directive is honoured only when the model *speaks* it and not when it *quotes* it: it must start at column 0 of a line outside any code fence, so quoted, fenced, indented, blockquoted and inline-code copies are refused and counted. Set `STICKIES_NO_GLOBAL_CAPTURE=1` to remove the `global` exception entirely (such directives are then filed under `cwd`). At most 20 notes are written per turn with the overflow reported, and the hook carries a 15s timeout. Storing a note is not code execution. The hook is non-blocking and never emits a `block` decision, so it can't cause a stop loop. It trusts `transcript_path`/`cwd` from the (local, Claude-Code-issued) hook event. |
...
| **Auto-sync** (`maybeAutoSync` in `src/git-sync.js`) | Doubly gated: runs only when **both** `$STICKIES_AUTO_SYNC` is truthy **and** `$STICKIES_SYNC_REPO` is set. When on, the SessionStart hook pulls and the Stop hook **commits** (only on turns that captured a new sticky) — it does **not** push unless `STICKIES_SYNC_PUSH=1` is also set, so by default nothing your notes are written into ever leaves the machine over the network. Best-effort: any failure is swallowed so it can't break a session. Off by default — no automatic git/network activity unless you opt in. |
| **Git sync** (`src/git-sync.js`, `src/sync.js`) | Opt-in and off by default — does nothing unless `$STICKIES_SYNC_REPO` points at a git working copy you own. All git calls use argument arrays (never a shell string), so repo paths/commit messages can't inject shell commands. A pull only happens if the repo has a remote you configured, and a PUSH additionally requires `STICKIES_SYNC_PUSH=1` — without it every sync ends at the local commit, so with no remote, or no push opt-in, it is purely local. **The exported sync document is plaintext JSON** containing all note bodies — store it in a **private** repo. Secret redaction still applies on write, but treat the repo as containing your notes. Merge is last-writer-wins by `updated_at`; a malicious/edited sync file can only add/replace stickies (data), not execute code, and bad records are skipped. Records arriving from a sync document are redacted on import as well as on write, so a hand-edited or older-build document cannot re-import raw secrets into the local store. Verified by attack: `__proto__` and `constructor` keys do not reach `Object.prototype`, unknown categories/importances/statuses are refused, oversized content is capped on import, and a path carrying markup characters is skipped rather than becoming a global note. **Two consequences of the merge rule are worth stating plainly, because they follow from the design rather than from a bug: anyone who can write to your sync repo can (a) revive notes you dismissed, since last-writer-wins by `updated_at` applies to status too, and (b) place a note in _every_ project on your machine, since a null `project_path` means global. Both are the same trust you extend by sharing the repo.** |
