...
//
// NB: not the entry point — `src/server.js` is, and it loads the node:sqlite warning suppressor
// before importing this module. Run this file directly and the warning comes back.
...
server.registerTool(
  'stickies_write',
  {
    ...
    inputSchema: {
      ...
      project_path: z
        .string()
        // A project path is stored, then later rendered into the dashboard's pages. The value
        // arrives from whatever the calling agent says, so it is untrusted input: refuse the
        // characters no real path contains rather than relying on every downstream renderer.
        .refine((v) => !/[<>\u0000-\u001f\u007f\u2028\u2029]/.test(v), {
          message: 'project_path contains characters that are not valid in a filesystem path',
        })
        .nullish()
